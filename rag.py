...
class RAGPipeline:
    ...
    def retrieve(self, query: str, top_k: int = 4, index=None, chunks=None, metadata=None) -> list[dict]:
        """
        Retrieves top_k chunks. Pass index/chunks/metadata explicitly to search
        a different (e.g. per-user, in-memory) index instead of the default disk one.
        """
    ...
    def query(self, question: str, top_k: int = 4, index=None, chunks=None, metadata=None) -> dict:
